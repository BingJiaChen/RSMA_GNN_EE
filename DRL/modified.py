...
def cal_loss(W, Theta, H, channel_bs_user,Pmax,mu,Pc,min_rate,e_rc,M,N,L,K):#array dimension require modified 
    ...
    beta = 1
    sum_rate = torch.sum(private_rate,dim=1) + common_rate
    power = mu[:,0]*Pmax + Pc
    EE = sum_rate / power
...
def generate_channel(M,N,L,K,RSMA_batch_size,LOS_bs_ris,sigma):
    ...
    return H, channel_bs_user, H, channel_bs_user, loc_user.flatten()
...
def gentrainchannel(M, N, L, K, RSMA_batch_size, LOS_bs_ris):
    ...
    H, D,_, _, loc_user = generate_channel(M, N, L, K, RSMA_batch_size, LOS_bs_ris, 0)
...
class RSMAEnv():
    ...
    def __init__(self, state_dim, action_dim, max_action, rho,M,N,L,K, steps):
        ...
        self.channel, self.state = gentrainchannel(M, N, L, K, 1, self.LOS_bs_ris)
    ...
    def step(self, action):
        self.time_step += 1
        reward, sumrate = self.getReward(action, self.state, self.channel)
        self.channel, self.state = self.ARNextChannel(self.state)
        next_reward, next_sumrate = self.getReward(action, self.state, self.channel)
        done = self.time_step >= self.steps
        return self.state, reward, done, sumrate, next_reward

    def getReward(self, action, state, channel):
        H, channel_bs_user = genrewardchannel(channel,self.M,self.N,self.L,self.K)
    ...
    def ARNextChannel(self, state):
        channel, state = gentrainchannel(self.M, self.N, self.L, self.K, 1, self.LOS_bs_ris)
        self.channel = self.channel*self.rho + channel*np.sqrt(1-self.rho**2)
        self.state = self.state*self.rho + state*np.sqrt(1-self.rho**2)
...
class Actor(nn.Module):
    def __init__(self, state_dim, action_dim, hidden_dim=256):
        ...
        self.fc3 = nn.Linear(hidden_dim*2+1, action_dim)
...
class ReplayBuffer:
    ...
    def push(self, state, action, reward, next_state, done, next_reward):
        self.memory.append((state, action, reward, next_state, done, next_reward))

    def sample(self, batch_size):
        batch = random.sample(self.memory, batch_size)
        state, action, reward, next_state, done, next_reward = zip(*batch)
        return (
            torch.stack(state).to('cuda'),
            torch.FloatTensor(action).to('cuda'),
            torch.tensor(reward, dtype=torch.float32).to('cuda'),
            torch.stack(next_state).to('cuda'),
...
class DDPGAgent:
    ...
    def select_action(self, state, reward, noise=0):
        # state = torch.tensor(state, dtype=torch.float32).unsqueeze(0)
        state = state.unsqueeze(0)
        action = self.actor(state,reward).detach().cpu().numpy()[0]
        action += noise
        return action
    ...
    def train(self):
        ...
        states, actions, rewards, next_states, dones, next_reward = self.replay_buffer.sample(self.batch_size)
        # Compute target Q-values
        with torch.no_grad():
            target_actions = self.target_actor(next_states, next_reward)
            target_Q = self.target_critic(next_states, target_actions, next_reward)
        ...
        Q = self.critic(states, actions, rewards)
        critic_loss = nn.MSELoss()(Q, target_Q)
        critic_loss.backward()
        self.critic_optimizer.step()

        # Update actor
        actor_loss = -self.critic(states, self.actor(states, rewards), rewards).mean()
    ...
    def train_ddpg_with_optuna(self,trial):
        ...
        for episode in range(num_episodes):
            ...
            while True:
                ...
                action = agent.select_action(state, reward, noise)
                next_state, reward, done, sumrate, next_reward = env.step(action)
                next_state = torch.FloatTensor(next_state).to('cuda')
                agent.replay_buffer.push(state, action, reward, next_state, done, next_reward)
                state = next_state
...
if __name__ == "__main__":
    ...
    RSMA_batch_size = 1
    state_dim = K*2
    action_dim = 2*L*N + M * (K + 1) * 2 + 2 + K #correction required
    max_action = 1
    steps = 20
    mu = 0
    noiseSigma = 1
    ...
    agent = DDPGAgent(state_dim, action_dim, max_action, 1e-4,1e-4,0.9,0.01,128)
    #Hyperparameters

...
def generate_channel(M,N,L,K,batch_size,LOS_bs_ris,sigma):
    ...
    sigma = 0.001
    H_imperfect1 = np.zeros((batch_size,M,N,L,K),dtype=np.complex128)
    if sigma > 0:
        imperfect_bs_user1 = np.random.normal(0,sigma,size=channel_bs_user.shape) + 1j*np.random.normal(0,sigma,size=channel_bs_user.shape)
        imperfect_ris_user1 = np.random.normal(0,sigma,size=channel_ris_user.shape) + 1j*np.random.normal(0,sigma,size=channel_ris_user.shape)
        imperfect_bs_user1 = imperfect_bs_user1 + channel_bs_user
        imperfect_ris_user1 = imperfect_ris_user1 + channel_ris_user
        for k in range(K):
            for l in range(L):
                channel_ris_user_tmp = imperfect_ris_user1[:,l,k,:,:]
                combined_ris = np.matmul(channel_bs_ris[:,l,:,:].transpose(0,2,1),channel_ris_user_tmp)
                H_imperfect1[:,:,:,l,k] = combined_ris
    sigma = 0.01
    H_imperfect2 = np.zeros((batch_size,M,N,L,K),dtype=np.complex128)
    if sigma > 0:
        imperfect_bs_user2 = np.random.normal(0,sigma,size=channel_bs_user.shape) + 1j*np.random.normal(0,sigma,size=channel_bs_user.shape)
        imperfect_ris_user2 = np.random.normal(0,sigma,size=channel_ris_user.shape) + 1j*np.random.normal(0,sigma,size=channel_ris_user.shape)
        imperfect_bs_user2 = imperfect_bs_user2 + channel_bs_user
        imperfect_ris_user2 = imperfect_ris_user2 + channel_ris_user
        for k in range(K):
            for l in range(L):
                channel_ris_user_tmp = imperfect_ris_user2[:,l,k,:,:]
                combined_ris = np.matmul(channel_bs_ris[:,l,:,:].transpose(0,2,1),channel_ris_user_tmp)
                H_imperfect2[:,:,:,l,k] = combined_ris

    return H_imperfect1, imperfect_bs_user1, H_imperfect2, imperfect_bs_user2, H, channel_bs_user
...
def cal_loss(W, Theta, H, channel_bs_user,Pmax,mu,Pc,min_rate,e_rc):
    batch_size = W.shape[0]
    M = W.shape[2]//2
    W_re = W[:,:,:M]
    W_im = W[:,:,M:]
    # W_temp = W_re[0] + 1j*W_im[0]
    # print(mu[0,0]*Pmax)
    # print(torch.trace(torch.matmul(W_temp,torch.conj(W_temp).transpose(1,0))))
    ...
    beta = 1
    sum_rate = torch.sum(sum_rate,dim=1)
    power = mu[:,0]*Pmax + Pc
    EE = sum_rate / power

    loss = -torch.mean(EE) - torch.mean(threshold)

...
def SCA(H, channel_bs_user, perfect_H, perfect_bs_user):
    H = H[0]
    channel_bs_user = channel_bs_user[0]
    perfect_H = perfect_H[0]
    perfect_bs_user = perfect_bs_user[0]
    # initialization ------------------------------------------------------ 
    ...
    try:
        ...
        # Dinkelbach -----------------------------------------------------------------------------------
            ...
            for k in range(K):
                for i in range(K+1):
                    concat_H = perfect_H[:,:,:,k].reshape((M,-1)).transpose()
                    t[k,i,:] = np.dot(concat_H,W[i,:].reshape((M,1))).squeeze()
                    g[k,i] = np.dot(perfect_bs_user[k].reshape((1,-1)),W[i].reshape((-1,1))).squeeze()
...
for n_iter in range(max_iter):
    ...
    H1, channel_bs_user1, H2, channel_bs_user2, perfect_H, perfect_bs_user = generate_channel(M,N,L,K,1,LOS_bs_ris,0)
    # EE1, rate1 = SCA(H1, channel_bs_user1, perfect_H, perfect_bs_user)
    EE2, rate2 = SCA(H2, channel_bs_user2, perfect_H, perfect_bs_user)   
    # start_time = time.time() 
    EE_perfect, rate_perfect = SCA(perfect_H, perfect_bs_user, perfect_H, perfect_bs_user)    
    # end_time = time.time()
    # total_time += end_time-start_time
    if EE_perfect > 10:
        # sum_EE_perfect.append(EE_perfect)
        # sum_rate_perfect.append(rate_perfect)
        # sum_EE1.append(EE1)
        # sum_rate1.append(rate1)
        sum_EE2.append(EE2)
        sum_rate2.append(rate2)
        # print("EE:",EE_perfect,"bps/Hz/Joule")
        # print("Sum rate:",rate_perfect,"bps/Hz/Joule")
# print("execution time:",total_time/max_iter)
# sum_EE1 = np.array(sum_EE1)
# sum_rate1 = np.array(sum_rate1)
sum_EE2 = np.array(sum_EE2)
sum_rate2 = np.array(sum_rate2)
# sum_EE_perfect = np.array(sum_EE_perfect)
# sum_rate_perfect = np.array(sum_rate_perfect)

# print("average EE:",np.mean(sum_EE_perfect),"bps/Hz/Joule")
# print("average sum rate:",np.mean(sum_rate_perfect),"bps/Hz/Joule")
# print("average EE:",np.mean(sum_EE1),"bps/Hz/Joule","sigma=0.001")
# print("average sum rate:",np.mean(sum_rate1),"bps/Hz/Joule","sigma=0.001")
print("average EE:",np.mean(sum_EE2),"bps/Hz/Joule","sigma=0.01")
print("average sum rate:",np.mean(sum_rate2),"bps/Hz/Joule","sigma=0.01")

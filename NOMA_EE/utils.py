...
def cal_loss(W, Theta, H, channel_bs_user,Pmax,mu,Pc,min_rate):
    ...
    concat_H = concat_H.squeeze()
    P = torch.norm(W,dim=2)
    sorted_P, indices = torch.sort(P)
    W_sort = torch.zeros(W.shape).to('cuda')
    H_sort = torch.zeros(concat_H.shape).to('cuda')
    for i in range(batch_size):
        W_sort[i] = W[i][indices[i]]
        H_sort[i] = concat_H[i][indices[i]]
    W_re = W_sort[:,:,:M]
    W_im = W_sort[:,:,M:]
    private_rate = torch.zeros((batch_size,K)).to('cuda')
    
    for m in range(K):
        SINR = torch.zeros((batch_size,m+1)).to('cuda')
        for k in range(m+1):
            A = H_sort[:,k,:].unsqueeze(2)
            signal_power = []
            ...
            gamma = signal_power[-1]/(sum_power-signal_power[-1]+sigma)
            SINR[:,k] = gamma
        private_rate[:,m] = torch.min(SINR,dim=1)[0]
    ...
    loss = -torch.mean(EE) - beta*(torch.mean(threshold))
